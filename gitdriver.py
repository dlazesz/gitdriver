...
def get_last_commit_date():
    ...
    if out:
        return datetime.strptime(out.decode(encoding='UTF-8').strip(),
...
def git_main(gd, docid, mime_types_to_download, raw, preview):
    ...
    print('Create repository "{title}"'.format(title=md['title']))
...
def unauth_main(docid, mime_types_to_download, preview):
    ...
    for ext, mime_type in mime_types_to_download.items():
        ...
        conn = urllib.request.urlopen(export_url_w_ext)
...
if __name__ == '__main__':
    opts = parse_args()
    if not opts.noauth:
        cfg = yaml.safe_load(open(opts.config, 'r', encoding='UTF-8'))
        gd = GoogleDrive(
